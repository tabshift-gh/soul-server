...
const listTableRows = async (req, res, next) => {
  ...
  */
	...
	const { _page = 1, _limit = 10, _search, _ordering, _schema, _extend, _filters = '' } = req.query
	...
	let orderString = ''
	if (_ordering) {
		orderString += ` ORDER BY ${_ordering.replace('-', '')} ${_ordering.startsWith('-') ? 'DESC' : 'ASC'}`
	...
	if (_extend) {
		...
		extendFields.forEach((extendedField) => {
			try {
				...
				if (!foreignKey) {
					throw new Error(`Foreign key not found for extended field '${extendedField}'`)
				}

				const { table: joinedTableName } = foreignKey

				const joinedTableFields = db.prepare(`PRAGMA table_info(${joinedTableName})`).all()
	...
	try {
		...
		const nextPage =
			data.length === limit
				? `/tables/${tableName}/rows?${params}_limit=${_limit}&_page=${page + 1}`
				: null
		const previous =
			page > 1 ? `/tables/${tableName}/rows?${params}_limit=${_limit}&_page=${page - 1}` : null
...
const insertRowInTable = async (req, res, next) => {
  ...
  */
	...
	const fields = Object.fromEntries(Object.entries(queryFields).filter(([, value]) => value !== null))
